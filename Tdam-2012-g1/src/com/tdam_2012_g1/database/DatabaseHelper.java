...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public String getNameContact(String Userid){
		...
		Cursor cursor = db.rawQuery("SELECT * FROM " + contactosAplicacionTable + " where " + colIdContact +
				" = ? ", values);
	...
	public Cursor getAllMensajes() {
	 ...
	 public ArrayList<MensajeWeb> getAllMensajeWeb(){
			...
			while (cursor.moveToNext()) {
				...
				aux.set_destinatario(cursor.getString(cursor.getColumnIndex(colIdContactoDestinatario)));
				
				aux.set_remitente(cursor.getString(cursor.getColumnIndex(colIdContactoRemitente)));
	 ...
	 public ArrayList<Conectividad> getAllConectividad(){
			...
			if(cursor.getCount() > 0) {
				...
				while (cursor.moveToNext()) {
					...
					aux.set_conexion(cursor.getString(cursor.getColumnIndex(colConexion)));
	 ...
	 public ArrayList<Mail> getMailsContacto(Contacto contact){
			...
			Cursor cursor = db.rawQuery("SELECT * FROM " + mailsTable + " where " + colIdUsuarioRemitente + " = ? ",
					selection);
	 ...
	 public boolean contactExist(String parName, String parPass){
		 ...
		 Cursor cursor = db.rawQuery("SELECT * FROM " + usuariosTable + " WHERE " + colNombre + " = ? " + 
					" and " + colContraseña + " = ? ",	values);
