...
//vista
	...
	public void onClick(View v) {
		...
		//Control de password
		if(password.getText().toString().equals(Repassword.getText().toString()) && password.getText().toString().getBytes().length >= 6){
	...
	private void Registrar_User(Usuario parUsuario){
		...
		if(hayconexionInternet){
			if(validarDatosIngresados()==true)
			{
				...
				RegisterUserTask _initTask = new RegisterUserTask(parUsuario,this);
				_initTask.execute();
	...
	/*
	 * Valida los datos del usuario para saber si el mismo ha introducido los datos correctos
	 * 
	 */
	...
	/*
	 * Clase que realiza una tarea asincrona de registar un nuevousuario en el dispositivo
	 */
    protected class RegisterUserTask extends AsyncTask<String, Integer, WebServiceInfo>
    {
		...
		protected WebServiceInfo doInBackground(String... params) {
			...
			if (result != null && WebServiceInfo.SUCCESS == result.getCode()) {
				//registra un nuevo usuario en la base de datos si el contacto se ha registrado correctamente
				SingletonDB.getInstance(context).getDatabaseHelper().addUsuario(user);
